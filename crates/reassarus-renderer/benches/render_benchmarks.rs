...
fn benchmark_collision_detection(c: &mut Criterion) {
    use reassarus_renderer::collision::{BoundingBox, CollisionResolver, PositionedEvent};
...
fn benchmark_animation_evaluation(c: &mut Criterion) {
    use reassarus_renderer::animation::{
...
fn benchmark_comparison(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("reassarus_renderer_vs_libass");

    // Our renderer
    group.bench_function("reassarus_renderer", |b| {
        b.iter(|| renderer.render_frame(black_box(&script), black_box(500)))
    });
