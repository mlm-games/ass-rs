...
//! [`RegistryIntegration`].
//!
//! Wires the handler factories and section processors shipped by reassarus-core into
...
impl RegistryIntegration {
    /// Register all built-in tag handlers from reassarus-core
    pub fn register_builtin_handlers(&mut self) -> Result<()> {
        use reassarus_core::plugin::tags::*;
    ...
    /// Register built-in section processors from reassarus-core
    pub fn register_builtin_sections(&mut self) -> Result<()> {
        use reassarus_core::plugin::sections::*;
