...
impl RemoveTagCommand {
    ...
    /// Process content inside override brackets using reassarus-core's parser
    fn process_override_content(&self, content: &str) -> String {
        use reassarus_core::analysis::events::tags::parse_override_block;

        if let Some(pattern) = &self.tag_pattern {
            // Remove only specific pattern using reassarus-core's parser
            ...
            // Process tags from reassarus-core parser
            for tag in &tags {
