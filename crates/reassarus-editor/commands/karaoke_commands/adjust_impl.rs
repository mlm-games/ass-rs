...
impl AdjustKaraokeCommand {
    /// Adjust karaoke timing in text using reassarus-core's ExtensionRegistry system
    fn adjust_karaoke_timing(&self, text: &str) -> Result<String> {
        use reassarus_core::analysis::events::tags::parse_override_block_with_registry;
        use reassarus_core::plugin::{tags::karaoke::create_karaoke_handlers, ExtensionRegistry};
        ...
        while let Some(ch) = chars.next() {
            if ch == '{' {
                ...
                // Use reassarus-core's registry-based parser
                ...
                // Process karaoke tags using reassarus-core's validated data
    ...
    fn adjust_karaoke_tags_with_registry(
        ...
        tags: &[reassarus_core::analysis::events::tags::OverrideTag],
        ...
        for tag in tags.iter().rev() {
            if tag.name().starts_with('k') {
                // This tag was validated by reassarus-core's karaoke handlers
                let tag_name = tag.name();
                let args = tag.args();

                // Extract duration from args (reassarus-core already validated this)
