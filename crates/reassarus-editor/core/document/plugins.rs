...
impl EditorDocument {
    ...
    pub fn registry(&self) -> Option<&reassarus_core::plugin::ExtensionRegistry> {
    ...
    pub fn register_tag_handler(
        ...
        handler: Box<dyn reassarus_core::plugin::TagHandler>,
    ) -> Result<()> {
    ...
    pub fn register_section_processor(
        &mut self,
        extension_name: String,
        processor: Box<dyn reassarus_core::plugin::SectionProcessor>,
    ) -> Result<()> {
        if self.registry_integration.is_none() {
            self.initialize_registry()?;
        }
