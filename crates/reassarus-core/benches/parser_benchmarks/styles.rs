...
";
    ...
    let script = reassarus_core::Script::parse(script_with_many_styles).unwrap();
    ...
    group.bench_function("style_conflict_detection", |b| {
        let analysis = reassarus_core::analysis::ScriptAnalysis::analyze(&script).unwrap();
        b.iter(|| {
...
pub fn bench_overlap_detection(c: &mut Criterion) {
    ...
    group.bench_function("overlap_50_events", |b| {
        let script = reassarus_core::Script::parse(&small_script).unwrap();
        let analysis = reassarus_core::analysis::ScriptAnalysis::analyze(&script).unwrap();

        b.iter(|| {
            black_box(reassarus_core::analysis::find_overlapping_dialogue_events(
    ...
    group.bench_function("overlap_200_events", |b| {
        let script = reassarus_core::Script::parse(&medium_script).unwrap();
        let analysis = reassarus_core::analysis::ScriptAnalysis::analyze(&script).unwrap();
    ...
    group.bench_function("overlap_1000_events", |b| {
        let script = reassarus_core::Script::parse(&large_script).unwrap();
        let analysis = reassarus_core::analysis::ScriptAnalysis::analyze(&script).unwrap();
