...
- **`crates/ass-core`** (v0.1.1) — **Complete.** Zero-copy parser, tokenizer
  (with optional SIMD path), analysis/linting engine, and trait-based plugin
  system. The mature core of the project.
- **`crates/ass-editor`** (v0.1.1) — **Complete.** Rope-based interactive editing
  APIs: undo/redo, fluent command layer, multi-document sessions, format
  conversion (SRT/WebVTT/ASS), and search indexing. `no_std`-capable.
- **`crates/ass-renderer`** (v0.1.0) — **Work in progress.** Software backend
  (tiny-skia) is functional; Vulkan/Metal/WebGPU backends are experimental
  scaffolding. Most outstanding TODOs in the repo live here.

Planned-but-not-yet-present crates (referenced in `/architecture` docs):
`ass-cli`, `ass-wasm`, `ass-benchmarks`.
...
#### Feature Flags (ass-renderer)
- **`default`/`full`**: `analysis-integration`, `software-backend`, `backend-probing`, `simd`, `image`, `serde`
- **`software-backend`**: CPU rasterizer via tiny-skia (functional)
- **`hardware-backend`** (`vulkan`, `metal`): experimental GPU backends
- **`web-backend`** (`wasm`): experimental WebGPU/WebGL backend
- **`libass-compare`**: stub, currently does not build — excluded from defaults (see Renderer note above)
